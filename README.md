# Inteligência Artificial

Repositório destinado aos trabalhos, exercícios e notebooks desenvolvidos durante a disciplina de Inteligência Artificial.

## Conteúdo

### Exercícios Python

Contém os exercícios desenvolvidos durante a disciplina para praticar conceitos básicos de programação em Python, como:

* Variáveis e entrada de dados;
* Estruturas condicionais;
* Estruturas de repetição;
* Listas e dicionários;
* Funções;
* Manipulação de strings;
* Recursividade;
* Algoritmos de ordenação e busca;
* Manipulação de matrizes;
* Outros exercícios de lógica de programação.

## Arquivos

| Arquivo                   | Descrição                                                                     |
| ------------------------- | ----------------------------------------------------------------------------- |
| `Exercícios_Python_Alexandre Henrique_6°D._Manhã.ipynb` | Notebook contendo os exercícios de Python desenvolvidos durante a disciplina. |

## Disciplina

**Disciplina:** Inteligência Artificial

**Curso:** Engenharia de Software

**Instituição:** Universidade Mogi das Cruzes (UMC)

Este repositório será atualizado conforme novos notebooks e atividades forem desenvolvidos durante a disciplina.
